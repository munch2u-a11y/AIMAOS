...
class BeliefStore:
    ...
    def record_usage(self, injected_ids: List[str],
                     lexicon_hit_ids: Optional[List[str]] = None,
                     when: Optional[datetime] = None) -> int:
        """Usage telemetry for Layer-3 (core belief) promotion.

        Every belief that made it into an injection bumps `access_count`;
        the subset that got there via a DIRECT lexicon match (its term or
        alias literally appeared in the trigger text — names, persona
        concepts, routine anchors) also bumps `lexicon_hits` and stamps
        `last_hit_at`. Heavily-relied-on beliefs surface as high
        lexicon_hits over time; run_core_promotion reads these counters.

        Batched: one file write per touched category. Raw memory chunks
        are skipped — they aren't Layer 2, and memory.json is the one
        category file big enough that rewriting it per pulse would hurt.
        Deliberately does NOT bump self.version: counters are telemetry,
        not content, and a version bump would defeat the vector-store
        sync fast path every single pulse.
        """
        if not self._cache_loaded:
            self.load_into_cache()
        injected = set(injected_ids or [])
        hits = set(lexicon_hit_ids or [])
        stamp = (when or datetime.now()).astimezone().isoformat(timespec="seconds")
        touched: Dict[str, None] = {}
        recorded = 0
        for bid in injected | hits:
            belief = self._beliefs_cache.get(bid)
            if not belief or belief.get("_category") == "memory":
                continue
            if bid in injected:
                belief["access_count"] = int(belief.get("access_count", 0)) + 1
            if bid in hits:
                belief["lexicon_hits"] = int(belief.get("lexicon_hits", 0)) + 1
                belief["last_hit_at"] = stamp
            touched[belief["_category"]] = None
            recorded += 1
        for category in touched:
            cat_beliefs = [b for b in self._beliefs_cache.values()
                           if b.get("_category") == category]
            self._write_category(category, cat_beliefs)
        return recorded
