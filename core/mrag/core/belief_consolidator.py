...
"""
    ...
    # ── Layer-3 core-belief promotion (human-gated) ──────────────────────

    def run_core_promotion(self, min_lexicon_hits: int = 5,
                           top_n: int = 12) -> Dict[str, Any]:
        """Nominates the most heavily relied-on Layer-2 beliefs as
        core-belief candidates — the Layer-3 tier that would eventually
        feed deep-sleep weight training.

        "Heavily relied on" is measured, not judged: `lexicon_hits`
        counts injections that happened because the belief's term/alias
        literally appeared in the trigger text (names — including the
        agent's own — persona concepts, routine anchors), maintained by
        PreGenerativeInjector via BeliefStore.record_usage. Raw memory
        chunks and machine-derived concept entries are never candidates.

        This pass does NOT alter the store. Nominations are merged into
        `core_beliefs_pending.json` in the store's data dir for human
        review — edit the file's `status` field ("pending" → "approved"
        or "rejected"). Existing statuses are preserved across runs;
        rejected entries are never re-nominated, and repeat nominations
        just refresh the counters. Nothing downstream (training-set
        export) may consume entries that aren't "approved".
        """
        if not self._store._cache_loaded:
            self._store.load_into_cache()
        ...
        candidates = [
            ...
            and int(b.get("lexicon_hits", 0)) >= min_lexicon_hits
        ]
        candidates.sort(key=lambda b: (int(b.get("lexicon_hits", 0)),
                                       int(b.get("access_count", 0))),
                        reverse=True)
        candidates = candidates[:top_n]

        path = os.path.join(self._store.data_dir, "core_beliefs_pending.json")
        try:
            with open(path) as f:
        ...
        for belief in candidates:
            bid = belief.get("id")
            entry = existing.get(bid)
            if entry:
                entry["lexicon_hits"] = int(belief.get("lexicon_hits", 0))
                entry["access_count"] = int(belief.get("access_count", 0))
                entry["last_nominated"] = stamp
                continue
            existing[bid] = {
                "id": bid,
                "status": "pending",
                ...
                "lexicon_hits": int(belief.get("lexicon_hits", 0)),
                "access_count": int(belief.get("access_count", 0)),
                "first_nominated": stamp,
                "last_nominated": stamp,
            }
            nominated += 1

        ordered = sorted(existing.values(),
                         key=lambda e: int(e.get("lexicon_hits", 0)), reverse=True)
        with open(path, "w") as f:
            json.dump(ordered, f, indent=2)

        stats = {"candidates": len(candidates), "new_nominations": nominated,
                 "pending_total": sum(1 for e in ordered if e.get("status") == "pending"),
                 "approved_total": sum(1 for e in ordered if e.get("status") == "approved"),
                 "path": path}
        logger.info(f"Core-belief promotion: {stats}")
        return stats
...
1. Write in third person, naming the specific person from the record (or "the user" /
   "the assistant" when unnamed).
...
4. "term": the single most distinctive anchor word or short phrase for this belief — a person's name,
   an activity, a place, a project. Required.
...
{{
  "beliefs": [
    {{"content": "...", "term": "...", "aliases": ["..."], "source_indices": [3, 7],
      "category": "...", "instance": "...", "subject": "..."}}
  ]
}}
