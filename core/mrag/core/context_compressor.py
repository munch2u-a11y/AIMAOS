...
class ContextCompressor:
    ...
    def __init__(
        ...
        Args:
            ...
            summary_max_tokens: Hard cap on the rolling summary body. Without a
                cap the summary grows with every compression cycle — harmless in
                a 200k window, fatal in a 12k one, where after a few cycles the
                summary alone would consume the space the cap is meant to free.
                When exceeded, the summary is condensed with one extra LLM call
                and, as a last resort, truncated from the oldest end (safe:
                Layer 1 memory keeps every turn verbatim, so nothing is lost —
                only evicted from the active window).
            tail_max_messages: Most recent messages kept verbatim through a
                compression (up to tail_token_fraction of total tokens).
            tail_token_fraction: Fraction of total tokens the verbatim tail may
                occupy.
        """
        ...
        self.tail_max_messages = tail_max_messages
        self.tail_token_fraction = tail_token_fraction

        self._previous_summary: Optional[str] = None
        # Session counter, read by the dashboard.
    ...
    def compress(self, messages: List[Dict[str, Any]], is_end_of_session: bool = False) -> List[Dict[str, Any]]:
        ...
        # 1. Find tail boundary (save tail_max_messages turns up to
        # tail_token_fraction of total tokens)
        tail_start = n_messages
        accumulated_tail = 0
        limit_tokens = int(total_tokens * self.tail_token_fraction)

        for i in range(n_messages - 1, self.protect_first_n - 1, -1):
            msg_tokens = self._estimate_tokens(str(messages[i].get("content", "")))
            if (n_messages - i) > self.tail_max_messages:
                break
        ...
        # Injected-context messages (is_injected_context) are dropped outright:
        # they are re-derived from the memory store every turn, so summarizing
        # them would write retrieval output back into the conversation record
        # and double-count facts the store already holds.
        ...
        for msg in messages[self.protect_first_n:target_end]:
            ...
            elif msg.get("is_injected_context"):
                continue
            else:
...
Only output the recollection, no preamble."""
        ...
        try:
            ...
            summary_text = self._enforce_summary_cap(summary_text)
    ...
    def _enforce_summary_cap(self, summary_text: str) -> str:
        """Keeps the rolling summary under summary_max_tokens.

        First tries one condensation LLM call; if the result is still over
        (or the call fails), truncates from the oldest end. Truncation is
        acceptable here because the summary is a working-window convenience,
        not the record — every summarized turn already lives verbatim in
        Layer 1 memory and remains retrievable.
        """
        if not self.summary_max_tokens:
            return summary_text
        if self._estimate_tokens(summary_text) <= self.summary_max_tokens:
            return summary_text

        target_words = int(self.summary_max_tokens * 0.7)
        condense_prompt = (
            f"Condense this recollection to under {target_words} words. Drop "
            f"conversational filler and older resolved threads first; keep all "
            f"dates, times, names, numbers, direct quotes, and unresolved "
            f"threads exactly. Keep strict chronological order and the [USER]/"
            f"[MODEL] turn identifiers. Only output the condensed recollection, "
            f"no preamble.\n\nRECOLLECTION:\n{summary_text}"
        )
        try:
            condensed = self.llm(condense_prompt).strip()
            if condensed:
                summary_text = condensed
        except Exception as e:
            logger.warning(f"Summary condensation failed, falling back to truncation: {e}")

        if self._estimate_tokens(summary_text) <= self.summary_max_tokens:
            return summary_text

        # Last resort: drop the oldest content. Split on sentence-ish
        # boundaries and keep the most recent sentences that fit.
        import re
        sentences = [s for s in re.split(r'(?<=[.!?])\s+|\n+', summary_text) if s.strip()]
        marker = "[...earlier events compacted; the full record remains in memory...]"
        marker_tokens = self._estimate_tokens(marker)
        kept: List[str] = []
        kept_tokens = marker_tokens
        for sentence in reversed(sentences):
            s_tokens = self._estimate_tokens(sentence)
            if kept and kept_tokens + s_tokens > self.summary_max_tokens:
                break
            kept.append(sentence)
            kept_tokens += s_tokens
        kept.reverse()
        return f"{marker} {' '.join(kept)}"
