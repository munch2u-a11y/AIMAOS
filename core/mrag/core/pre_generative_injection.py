...
# Accepts bare numbers and short alpha-prefixed forms: "7", "t7" (chat
# turns), "p12" (pulse ids), "turn3". Without the prefix tolerance,
# pulse-stamped memories ("p12") silently fall out of adjacency
# expansion — the reserve budget goes unused and conversational
# neighbors never surface.
...
class PreGenerativeInjector:
    ...
    def __init__(
        ...
        # Session stat, read by the dashboard.
        self.last_injection_tokens = 0

        # Blacklist stores recent belief IDs to avoid repetitive injection
        self._recent_injections: List[str] = []
        self._last_lexicon_matches: set = set()
    ...
    def inject(self, trigger_text: str, current_context: str = "", limit: Optional[int] = None,
        ...
        self._last_lexicon_matches = set()
        ...
        # Usage telemetry: what was injected, and which of those were
        # direct lexicon hits (feeds Layer-3 core-belief promotion).
        injected_ids = [b.get("id") for b in beliefs if b.get("id")]
        hit_ids = [bid for bid in injected_ids if bid in self._last_lexicon_matches]
        try:
            self._belief_store.record_usage(injected_ids, hit_ids)
        except Exception as e:
            logger.warning(f"Usage telemetry failed: {e}")

        lines.append("------------------------")

        rendered = "\n".join(lines)
        self.last_injection_tokens = count_text_tokens(rendered)
        return rendered

    def inject_message(self, trigger_text: str, current_context: str = "",
                       limit: Optional[int] = None, current_time: Optional[Any] = None,
                       role: str = "user") -> Optional[Dict[str, Any]]:
        """inject(), packaged as a chat message dict tagged with
        is_injected_context=True. Harnesses that keep the injection in the
        message history (rather than weaving it into the prompt string)
        should append this form: the ContextCompressor recognizes the tag
        and excludes the injection from rolling summaries — injections are
        re-derived from the store every turn, so summarizing them would
        double-count facts the store already holds. Returns None when no
        beliefs matched. Strip the tag key before sending to strict chat
        APIs (see DEVELOPER_NOTES)."""
        content = self.inject(trigger_text, current_context=current_context,
                              limit=limit, current_time=current_time)
        if not content:
            return None
        return {"role": role, "content": content, "is_injected_context": True}
    ...
    def _pull_relevant_beliefs(self, text: str, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        # Stashed for inject()'s usage telemetry: of the beliefs finally
        # injected, these arrived via direct keyword overlap (Layer-2
        # direct hits — the promotion signal for core beliefs).
        self._last_lexicon_matches = set(lexicon_ids)
        if lexicon_ids:
